...
export class AuthService {
  ...
  async signUpWithPassword(identifier: string, password: string, metadata: Record<string, any> = {}): Promise<SignUpResult> {
    try {
      const isEmail = identifier.includes('@');
      const authOptions = isEmail ? { email: identifier, password } : { phone: identifier, password };
  ...
  async signInWithPassword(identifier: string, password: string) {
    try {
      ...
      return await safeSupabaseOperation(async (client) => {
        const { data, error } = await withTimeout(
          client.auth.signInWithPassword(authOptions),
          TIMEOUT_CONFIG.AUTH_OPERATIONS,
          'SignIn'
        );
