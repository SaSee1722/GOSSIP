...
export function useAuth(): AuthContextType {
  ...
  const signUpWithPassword = async (identifier: string, password: string, metadata?: Record<string, any>): Promise<SignUpResult> => {
    context.setOperationLoading(true);
    try {
      const result = await authService.signUpWithPassword(identifier, password, metadata);
      return result;
    } catch (error) {
  ...
  const signInWithPassword = async (identifier: string, password: string): Promise<AuthResult> => {
    context.setOperationLoading(true);
    try {
